...
local function log(premature, conf, message)
  ...
  for _, metric_config in pairs(conf.metrics) do
    if metric_config.name ~= "status_count" 
      ...
      local gauge = logger[metric_config.stat_type]
      if stat_name ~= nil and gauge ~= nil and stat_value ~= nil then
        gauge(stat_name, stat_value, metric_config.sample_rate, logger, metric_config.tags)
      end
      
    else
      local gauge = gauges[metric_config.name]
      if gauge ~= nil then
        gauge(api_name, message, metric_config, logger, metric_config.tags)
      end
    end
  end
